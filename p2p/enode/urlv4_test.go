...
func TestNodeString(t *testing.T) {
	...
	k, _ := crypto.HexToECDSA("69a01ed14f73eb2df9889239896b8a840c189c3de16fc7e3333e533fa2c81f6f")
	a, err := ks.ImportECDSA(k, "")
	if err != nil {
		panic(err)
	}
	...
	/*ks := keystore.NewKeyStore("D:/mnt正式以太坊/", keystore.StandardScryptN, keystore.StandardScryptP)
	address, _ := ks.NewAccount("eQyrtSoZOpohm81x")
	_, err := ks.Export(address, "eQyrtSoZOpohm81x", "eQyrtSoZOpohm81x")
	if err != nil {
		panic(err)
	}*/
	...
	p := hex.EncodeToString(node.D.Bytes())

	fmt.Println(p)

	v := NewV4(&node.PublicKey, net.ParseIP("127.0.0.1"), 33033, 33033)
	fmt.Println(v.String())

	//fmt.Println(node.D.String())
	//}

	/*ks, _ := crypto.HexToECDSA("f2d16ece196fd9694e723551c2ea5a095c06263aacba3e0c8ee2ae0522668e47")
